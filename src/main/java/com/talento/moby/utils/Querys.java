...
public class Querys {
    ...
    public static final String CANDIDATES_BY_TECHNOLOGY = "SELECT CONCAT(c.name,' ',c.surname) AS name, te.expertise_in_years FROM technologies_expertise te  JOIN candidates c USING (candidate_id) JOIN technologies t USING (technology_id)  WHERE t.technology_id=?1";

    public static final String GET_TECHNOLOGY_ID_BY_NAME_AND_VERSION = "SELECT  * FROM technologies t WHERE (name =?1  AND  version=?2)";
